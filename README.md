...
```bash
git clone https://github.com/silverenternal/ai-excel-integration.git
...
- 后端服务：启动后端服务后，访问 `http://localhost:8080`
- 前端开发：在 `frontend` 目录下运行 `npm run dev`，访问 `http://localhost:5173`，前端会自动代理API请求到后端服务

#### 生产环境

- 构建后：运行 `mvn clean install` 后，前端会自动构建并打包到后端静态资源中，访问 `http://localhost:8080` 即可使用完整的全栈应用
- 独立运行：运行 `java -jar target/ai-excel-integration-*.jar` 后，访问 `http://localhost:8080`

#### 端口配置和网络问题解决

- 如果默认端口（如8080）被占用，可以在 `.env` 文件中修改 `SERVER_PORT` 变量
- 如果遇到连接问题，请检查：
  1. 确认端口未被防火墙阻止
  2. 检查是否有其他服务占用了指定端口
  3. 在 Windows 系统上，有时需要管理员权限运行应用
- 应用启动成功后，日志会显示 "Tomcat started on port XXXX"，表示服务已成功启动
